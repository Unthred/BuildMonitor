...
public static class LaunchProfileEnvironmentApplier
{
    ...
    public static IReadOnlyList<string> ResolveListenUrls(
        ...
        string? launchProfile)
    {
        var settings = string.IsNullOrWhiteSpace(launchProfile)
            ? null
            : TryLoadProfile(rootFolder, projectFile, launchProfile);

        if (settings is null || string.IsNullOrWhiteSpace(settings.ApplicationUrl))
        {
            return [];
        }
