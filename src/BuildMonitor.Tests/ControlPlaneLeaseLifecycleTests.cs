...
{
    [Fact]
    public void Intermediate_finalization_holds_exclusivity_while_cancel_is_retired()
    {
        ...
        // Step 1 — terminal / no longer cancellable; exclusivity still held.
        InvokeRetireCancellationTarget(env.Runtime, lease);

        Assert.Null(GetActiveLease(env.Runtime));
        Assert.Equal(1, GetPrivateField<int>(env.Runtime, "agentRebuildInProgress"));

        var cancelEx = Assert.Throws<InvalidOperationException>(
            () => env.Runtime.RequestCancelControlPlaneOperation(lease.OperationId));
        Assert.Contains("No cancellable", cancelEx.Message, StringComparison.OrdinalIgnoreCase);

        var acquireEx = Assert.Throws<TargetInvocationException>(
            () => InvokeAcquire(env.Runtime, ControlPlaneOperationKind.Tests));
        Assert.IsType<InvalidOperationException>(acquireEx.InnerException);
        Assert.Contains("already running", acquireEx.InnerException!.Message, StringComparison.OrdinalIgnoreCase);

        // Step 3 — after finalization, exclusivity releases and a new op may acquire.
        InvokeReleaseExclusivity(env.Runtime, lease, ControlPlaneOperationKind.Rebuild);

        Assert.Null(GetActiveLease(env.Runtime));
        Assert.Equal(0, GetPrivateField<int>(env.Runtime, "agentRebuildInProgress"));

        var next = InvokeAcquire(env.Runtime, ControlPlaneOperationKind.Tests);
        Assert.NotEqual(lease.OperationId, next.OperationId);
        InvokeReleaseExclusivity(env.Runtime, next, ControlPlaneOperationKind.Tests);
    }
    ...
    [Fact]
    public void No_old_new_overlap_until_exclusivity_released()
    {
        using var env = CreateRuntimeEnvironment();
        ...
        InvokeRetireCancellationTarget(env.Runtime, oldLease);

        // During finalization: still no new lease, cancel remains 409.
        Assert.Throws<InvalidOperationException>(
            () => env.Runtime.RequestCancelControlPlaneOperation(oldLease.OperationId));
        var midOverlap = Assert.Throws<TargetInvocationException>(
            () => InvokeAcquire(env.Runtime, ControlPlaneOperationKind.Rebuild));
        Assert.Contains("already running", midOverlap.InnerException!.Message, StringComparison.OrdinalIgnoreCase);

        InvokeReleaseExclusivity(env.Runtime, oldLease, ControlPlaneOperationKind.Tests);
        ...
        InvokeRetireCancellationTarget(env.Runtime, newLease);
        InvokeReleaseExclusivity(env.Runtime, newLease, ControlPlaneOperationKind.Rebuild);
    }
    ...
    public void Duplicate_cancel_while_genuinely_active_is_idempotent()
    {
        ...
        InvokeRetireCancellationTarget(env.Runtime, lease);
        InvokeReleaseExclusivity(env.Runtime, lease, ControlPlaneOperationKind.ShipCheck);
    ...
    [Fact]
    public void Never_leaves_exclusive_flag_zero_with_old_lease_installed()
    {
        using var env = CreateRuntimeEnvironment();
        var lease = InvokeAcquire(env.Runtime, ControlPlaneOperationKind.Rebuild);
        Assert.Equal(1, GetPrivateField<int>(env.Runtime, "agentRebuildInProgress"));
    ...
    private static void InvokeRetireCancellationTarget(
        ProjectRuntime runtime,
        ControlPlaneOperationLease lease)
    {
        var method = typeof(ProjectRuntime).GetMethod(
            "RetireControlPlaneCancellationTarget",
            BindingFlags.Instance | BindingFlags.NonPublic);
        Assert.NotNull(method);
        method!.Invoke(runtime, [lease]);
    }

    private static void InvokeReleaseExclusivity(
        ProjectRuntime runtime,
        ControlPlaneOperationLease lease,
        ControlPlaneOperationKind kind)
    {
        var method = typeof(ProjectRuntime).GetMethod(
            "ReleaseControlPlaneOperationExclusivity",
            BindingFlags.Instance | BindingFlags.NonPublic);
        Assert.NotNull(method);
