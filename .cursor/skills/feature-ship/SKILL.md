...
| User said | End state |
|-----------|-----------|
| **ship** / **ship it** / **merge** | PR **merged** to `main` + issue closed (`Closes #N`) |
| **ready for review** / **open PR** | Commit + push + active PR; issue linked |
...
4. **Git + GitHub** — per `work-tracking.mdc`: resolve `#<id>`; commit `#<id>: …`; push; PR body `Closes #<id>`.
5. **Merge** — only for full **ship it**: `gh pr merge --squash` after user confirms build/test.
