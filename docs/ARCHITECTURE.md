...
10. **Failure details** (`ProjectFailureDetailsBuilder`, `#111a`/`#111b`) answers “why unhealthy now?” from current Local build/test and Azure facet state (history enrichment by id only). Status panel **Failure details** sits above Recent activity; run-host slice follows later (`#111c`). See [features/failure-details.md](features/failure-details.md).
