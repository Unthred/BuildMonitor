...
**Azure monitoring (Slice 3B):** Active-in-session projects with Azure + ≥1 pipeline + valid connection are polled continuously. One Builds list request per selected pipeline per cycle (`api-version=7.1`, `$top=25`, `queryOrder=queueTimeDescending`). Active runs beat latest completed. Facets merge into `ProjectHealthSnapshot.Azure` through `HealthCoalescer` / `ProjectHealthComposer` (single tray rollup). Current Git branch is presentation focus only. Auth/network → Amber; cancelled/NoRun → Neutral; zero pipelines → Connected / Not monitored (no HTTP). In-memory facets only (notifications deferred). Cadence ≈45s settled, ≈8s while Azure active, 30→120s failure backoff.
