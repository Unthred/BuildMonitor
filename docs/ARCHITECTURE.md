...
10. **Failure details** (`ProjectFailureDetailsBuilder`, `#111a`) answers “why unhealthy now?” for current Local build/test failures, with history enrichment by id only. Status panel **Failure details** sits above Recent activity; Azure/run-host slices follow later. See [features/failure-details.md](features/failure-details.md).
