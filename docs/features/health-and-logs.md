...
| 9 | Auto-open log per project (`Never` / `Errors` / `Warnings` / `Always`). **Errors** opens on a newly completed failed build result, including watch rebuilds that stay `Watching`. **Rebuild & restart** never auto-opens or focuses the log window; output is still captured to that project's log file while the window is closed. | `App.AutoOpenLogsOnTransition`, `AutoOpenLogSession`, `AutoOpenLogTransitionEvaluator`, `BuildResultTransitionEvaluator` |
| 9b | Auto-show status panel while Local/Azure build activity (app-level, default **on** for both); edit-gating flows respect Local toggle. Deliberate dismiss during an activity hold suppresses further activity auto-show until that panel-wide hold settles (#132); hover/manual open still allowed | `App.SyncBuildActivityStatusPanelVisibility`, `StatusPanelVisibilityPolicy`, `StatusPanelActivityAutoShowCycle`, `StatusPanelBuildVisibilityEvaluator` |
| 9c | Per-project **Recent activity** timeline (operational history) under status-card actions; fuller list in Build diagnostics | `OperationalHistoryPresentationMapper`, `HoverStatusPanel`, `BuildDiagnosticsWindow` |
...
| 11 | **Rebuild & restart** runs full build then starts app. Does not open the log window. Uses `--no-launch-profile` plus profile environment variables and `ASPNETCORE_URLS` from the initiating project's captured context. | `ProjectRuntime.RestartAppCoreAsync(rebuildFirst: true)`, `ProjectRunContext`, `LaunchProfileEnvironmentApplier` |
